...
def _mock_check(project_dir, *extra_args):
    ...
    with (
        patch("py_cq.api.run_tools", return_value=[tr]),
...
def test_check_language_flag_python_runs_normally(project_dir):
    ...
    with (
        ...
        result = runner.invoke(
            app, ["check", str(project_dir), "--language", "python", "-o", "score"]
        )
    assert result.exit_code == 0
    assert "0.9" in result.output
...
def test_check_only_filters_tools(project_dir):
    ...
    combined = _fake_combined(str(project_dir))
    with (
        patch("py_cq.api.run_tools", return_value=[tr]) as mock_run,
        patch("py_cq.cli.aggregate_metrics", return_value=combined),
    ):
...
def test_check_only_and_skip_combined(project_dir):
    ...
    with (
        ...
        runner.invoke(
            app, ["check", str(project_dir), "--only", "ruff,ty,bandit", "--skip", "ty"]
        )
    passed = list(mock_run.call_args[0][0])
...
def test_check_exclude_merge(project_dir):
    ...
    with (
        ...
        runner.invoke(
            app, ["check", str(project_dir), "--exclude", "tests", "-o", "score"]
        )
...
def test_version_skips_extras_and_unknown_deps():
    ...
    with (
        patch("py_cq.cli.requires", return_value=fake_reqs),
        patch(
            "py_cq.cli.version",
            side_effect=["1.0.0", PackageNotFoundError("nonexistent-pkg")],
        ),
    ):
        result = runner.invoke(app, ["--version"])
    assert result.exit_code == 0
